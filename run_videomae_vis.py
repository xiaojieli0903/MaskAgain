...
def main(args):
    ...
    while run_time < args.times:
        ...
        with torch.no_grad():
            ...
            sav_path = f"{args.save_path}/rec_video_{run_time}.avi"
            videowrite = cv2.VideoWriter(sav_path, fourcc=fourcc, fps=4,
                                         frameSize=(224, 224))
            for id, im in enumerate(imgs):
                print(f'saving index {id}')
                im.save(f"{args.save_path}/rec_img{id}_{run_time}.jpg")
                im_save = np.array(im.convert("RGB"))[:, :, ::-1]
                im = Image.fromarray(np.uint8(im_save))
                for time in range(2):
                    videowrite.write(np.array(im))
