...
def train_distill(module_list,
    ...
    Align_feature = module_list[1]
    if opt.distill in ['hinTop','VTop']:
        Align_valuesum = module_list[2]
        Align_valuesum_t = module_list[3]
    if opt.distill in ['hybrid']:
        Align_valuesum = module_list[2]
        Align_valuesum_t = module_list[3]
        Align_valuesum1 = module_list[4]
        Align_valuesum1_t = module_list[5]
    ...
    for step, batch in enumerate(
        ...
        with torch.cuda.amp.autocast():
            ...
            if opt.distill == 'hinTop':
                ...
                v_s = v_list_s[-1]
                v_t = v_list_t[-1]
                loss_kd = criterion_kd(att_s, att_t, v_s,v_t,t=opt.kd_T,module=Align_valuesum,module_t=Align_valuesum_t)
                ...
                loss_kd1 = criterion_kd1(att_s1, att_t1, v_s,v_t,t=opt.kd_T,module=Align_valuesum,module_t=Align_valuesum_t)                
                loss_kd2 = criterion_kd2(att_s2, att_t2, v_s,v_t,t=opt.kd_T,module=Align_valuesum1,module_t=Align_valuesum1_t)
        ...
        module_list_parameters = chain(model.parameters(),Align_feature.parameters())
        if opt.distill in ['hinTop','VTop']:
            module_list_parameters = chain(module_list_parameters,Align_valuesum.parameters(),Align_valuesum_t.parameters())
        if opt.distill in ['hybrid']:
            module_list_parameters = chain(module_list_parameters,Align_valuesum.parameters(),Align_valuesum_t.parameters(),Align_valuesum1.parameters(),Align_valuesum1_t.parameters())
        is_second_order = hasattr(
            optimizer, 'is_second_order') and optimizer.is_second_order
        grad_norm = loss_scaler(loss_all,
                                optimizer,
                                clip_grad=max_norm,
                                parameters=module_list_parameters,
