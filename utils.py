...
def save_module(args,
    ...
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('checkpoint-%s.pth' % epoch_name)]
        for checkpoint_path in checkpoint_paths:
            if args.distill in ['hinTop','DeltaTop','hybrid','DC']:
                to_save = {
                    'model': model_without_ddp[0].state_dict(),
                    'Align_feature': model_without_ddp[1].state_dict(),
                    'Align_valuesum': model_without_ddp[2].state_dict(),
                    'optimizer': optimizer.state_dict(),
                    'epoch': epoch,
                    'scaler': loss_scaler.state_dict(),
                    'args': args,
    ...
    if loss_scaler is not None:
        ...
        if args.resume:
            if args.resume.startswith('https'):
                ...
                checkpoint = torch.load(args.resume, map_location='cpu')
            model_without_ddp[0].load_state_dict(checkpoint['model'])
            model_without_ddp[1].load_state_dict(checkpoint['Align_feature'])
            if args.distill in ['hinTop','DeltaTop','hybrid','DC']:
                model_without_ddp[2].load_state_dict(checkpoint['Align_valuesum'])
            print("Resume checkpoint %s" % args.resume)
