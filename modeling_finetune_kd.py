...
class Attention(nn.Module):
    ...
    def forward(self, x, is_att = False):
        ...
        if is_att:
            att =attn#（batch，nums_head,patch,patch）


        attn = attn.softmax(dim=-1)#维度不变

        attn = self.attn_drop(attn)#维度不变


        x = (attn @ v).transpose(1, 2).reshape(B, N, -1)#(batch_size,patch,D) hintop在这层

        x = self.proj(x)#维度不变 这之后就是特征了hintloss

        x = self.proj_drop(x)#维度不变 
...
class Block(nn.Module):
    ...
    def forward(self, x, is_att = False):
        if is_att:
            x_att, att, v = self.attn(self.norm1(x),is_att)
        else:
